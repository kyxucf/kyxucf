...
function swfReady() {
	...
	{
		running_version_number = movie.getParameter('VERSION');
		running_version = "SlideShowPro Player SWF <strong>" + running_version_number.replace(' (AS3)', '') + '</strong>';
	}
}
...
function getVersion() {
	...
	if (swf_ready && running_version_number < SlidePress.swfVersion) {
		...
		/* if (typeof(window.innerHeight) == 'number') {
			window.innerHeight = SlidePress.sspHeight * 1 + warning.height() + 52;
		} */
