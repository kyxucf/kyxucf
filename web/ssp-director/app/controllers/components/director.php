...
class DirectorComponent extends Object {
	...
	function fetchAccount($action = 'dummy') {
		...
		if (empty($account)) {
			...
			$account = $this->Account->find('first', array('conditions' => array('not' => array('version' => null))));
			$account['Account']['trial'] = (bool) empty($account['Account']['activation_key']);
			$to_cache = $account;
	...
	function ffmpeg() {
		if (function_exists('exec') && (DS == '/' || (DS == '\\' && FFMPEG_PATH_FINAL != 'ffmpeg'))) {
			...
			if (empty($out)) {
				...
				if (strpos(strtolower($out[0]), 'ffmpeg') !== false) {
					return true;
				} else {
					return false;
				}
