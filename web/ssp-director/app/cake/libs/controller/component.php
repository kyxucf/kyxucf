		...
		foreach ((array)$normal as $component => $config) {
			...
			if (isset($this->_loaded[$component])) {
				...
				if ($componentCn === 'SessionComponent') {
					if (isset($object->disableSessions)) {
						$continue = false;
						if (is_array($object->disableSessions)) {
							if (in_array($object->action, $object->disableSessions)) {
								$continue = true;
							}
						} elseif ($object->disableSessions) {
							$continue = true;
						}
						if ($continue) {
							foreach($this->_primary as $i => $name) {
								if ($name == 'Session') {
									unset($this->_primary[$i]);
								}
							}
							continue;
						}
